...
class PuzzleUI:
    def __init__(self):
        ...
        # NUEVO: modo de juego manual (cuando el usuario quiere mover fichas él mismo)
        self.play_mode = False  # True = el usuario juega manualmente (clic/teclas)
        ...
        # creo 4 Botones (en el sur, bajo el tablero) + 1 nuevo ("Jugar yo")
        self.buttons = {
            ...
            # NUEVO: botón para habilitar el modo de juego manual
            "play":       pygame.Rect(self.board_x - 150,  south_top + 120, 120, 38),
        }
        ...
        self.instructions = (
            ...
            "• JUGAR YO: mueve tú las fichas (clic en ficha adyacente al 0 o flechas)."
        )
    ...
    def draw(self):
        ...
        labels = {
            ...
            "solve":"Resolver",
            "clear":"Limpiar",
            "play":"Jugar yo"  # NUEVO: botón de juego manual
        }
        for key, rect in self.buttons.items(): # recorre los botones, key es el nombre, rect es el rectangulo definido antes

            # determina si el boton esta activo (modo edicion inicio/meta o modo juego)
            active = (key == "edit_start" and self.edit_mode == "start") or \
                     (key == "edit_goal"  and self.edit_mode == "goal")  or \
                     (key == "play"       and self.play_mode)
    ...
    # NUEVO: devuelve la posición (r,c) donde está el 0 (hueco)
    def _find_zero(self):
        """Devuelve (r,c) de la casilla 0."""
        for r in range(3):
            for c in range(3):
                if self.state[r][c] == 0:
                    return r, c
    ...
    # NUEVO: intenta mover una ficha (r,c) si es adyacente al 0
    def _try_move_tile(self, r, c):
        """Si (r,c) es adyacente al 0, intercambia y devuelve True; si no, False."""
        zr, zc = self._find_zero()
        if zr is None:
            return False
        # adyacencia Manhattan (arriba/abajo/izquierda/derecha)
        if abs(zr - r) + abs(zc - c) == 1:
            self.state[zr][zc], self.state[r][c] = self.state[r][c], self.state[zr][zc]
            return True
        return False

    # NUEVO: intentar mover el 0 por delta (usado por flechas)
    def _try_move_zero_by_delta(self, dr, dc):
        """Mueve el 0 por delta (dr,dc) si es válido."""
        zr, zc = self._find_zero()
        nr, nc = zr + dr, zc + dc
        if 0 <= nr < 3 and 0 <= nc < 3:
            self.state[zr][zc], self.state[nr][nc] = self.state[nr][nc], self.state[zr][zc]
            return True
        return False

    # NUEVO: chequea condición de victoria y resetea modos si se alcanzó la meta
    def _check_win(self):
        if self.state == self.solver.goal_state:
            self.message = "ganador"
    ...
    def handle_click(self, pos):
        ...
        # Tablero:
        # - si estoy en Editar Inicio: selecciono celda
        # - si estoy en Jugar yo: intento mover la ficha clickeada si es adyacente al 0
        if x >= self.panel_width and y < self.board_size:
            ...
            if self.edit_mode == "start":
                ...
                return
            if self.play_mode:
                if self._try_move_tile(row, col):  # intenta mover ficha hacia el 0
                    self.draw()                    # repinta para ver el movimiento
                    self._check_win()              # verifica si ya se alcanzó la meta
                return

        # Botones
        # Botón Editar Inicio
        if self.buttons["edit_start"].collidepoint(pos):
            self.play_mode = False                 # salir de jugar si estaba
        ...
        # Botón Jugar yo (NUEVO)
        if self.buttons["play"].collidepoint(pos):
            # Entra a modo juego manual: el usuario puede mover fichas con clic o flechas
            self.play_mode = True
            self.edit_mode = None
            self.selected_tile = None
            self.message = ""
        ...
        # verifica si estas en modo de edicion de inicio y si hay una celda seleccionada
        if self.edit_mode == "start" and self.selected_tile is not None:
            used = self._used_numbers(matrix=self.state, exclude=self.selected_tile) # llama a la funcion de numeros usados, y devuelve los numeros ya usados en el tablero

            # recorre todos los botones numericos (0-8)
            for i, rect in enumerate(self.num_buttons):
                if rect.collidepoint(pos): # comprueba si el clic fue dentro del boton y solo 1 de los 9 deberia detectarlo
                    if i in used: # si el numero esta en uso, no hace nada, entonces asegura que no se repitan los numeros en el tablero
                        return
                    # se obtiene la fila o columna seleccionaada, asigna el numero que se cliqueo.
    ...
    def handle_keydown(self, event):
        # Entrada de meta (modo goal)
        if self.edit_mode != "goal":
            # si no estoy editando meta, puedo checar controles de juego manual (abajo)
            pass
        else:
        ...
        # Controles de juego manual (modo play): flechas mueven el 0 (hueco)
        if self.play_mode:
            moved = False
            if event.key == pygame.K_UP:    # flecha ↑: subir el 0 (intercambia con casilla de arriba)
                moved = self._try_move_zero_by_delta(-1, 0)
            elif event.key == pygame.K_DOWN:
                moved = self._try_move_zero_by_delta(1, 0)
            elif event.key == pygame.K_LEFT:
                moved = self._try_move_zero_by_delta(0, -1)
